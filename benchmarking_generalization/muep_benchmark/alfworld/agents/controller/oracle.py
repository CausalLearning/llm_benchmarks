    ...
    def step(self, action_str):
        ...
        try:
            ...
            if cmd['action'] == self.Action.GOTO:
                ...
                obj, rel, tar = cmd['obj'], cmd['rel'], cmd['tar']
                object = self.get_object(obj, self.objects)
                assert self.env.last_event.metadata['inventoryObjects'][0]['objectId'] == object['object_id']
                recep = self.get_object(tar, self.receptacles)
                # print(self.env.last_event.metadata['inventoryObjects'][0]['objectId'])
                # print(recep['num_id'])
                # print(recep['object_id'])
                ...
                if event.metadata['lastActionSuccess']:
                    ...
                    print(f'You fail to put [{self.env.last_event.metadata["inventoryObjects"][0]["objectId"]}] to [{recep["num_id"]}]')
                    if recep['num_id'] == 'countertop 2':
                        recep = self.get_object('countertop 1', self.receptacles)
                        event = self.navigate(recep['locs'])
                        event = self.env.step({'action': "PutObject",
                                        'objectId': self.env.last_event.metadata['inventoryObjects'][0]['objectId'],
                                        'receptacleObjectId': recep['object_id'],
                                        'forceAction': True})
                        self.inventory.pop()
                        self.feedback = "You put the %s %s the %s." % (obj, rel, 'countertop 1')
                ...
                if not event.metadata['lastActionSuccess']:
                    # force action
                    self.env.set_toggle_on(obj['object_id'].split('|')[0])
                ...
                if all(e.metadata['lastActionSuccess'] for e in events) and self.curr_recep == tar:
                    ...
                    self.env.cleaned_objects = self.env.cleaned_objects | set([object['objectId']])
                else:
                    print("fail to clean due to failling to put it to sinkbasin")
                    event = self.env.step(dict(action='CleanObject', objectId=object['objectId']))
                    self.env.cleaned_objects = self.env.cleaned_objects | set([object['objectId']])
                    # print(self.env.cleaned_objects)
                    self.feedback = "You clean the %s using the %s." % (obj, tar)
