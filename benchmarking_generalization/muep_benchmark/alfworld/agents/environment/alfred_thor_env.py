...
class AlfredThorEnv(object):
    ...
    class Thor(threading.Thread):
        ...
        def set_task(self, task_file):
            ...
            print(f'Playing {self.traj_root}')
        ...
        def reset(self, task_file):
            ...
            if self.controller_type == 'oracle':
                self.controller = OracleAgent(self.env, self.traj_data, self.traj_root,
                                              load_receps=True, debug=debug,
                                              goal_desc_human_anns_prob=self.goal_desc_human_anns_prob, use_gt_relations=True)
    ...
    def get_env_paths(self):
        ...
        if self.train_eval != "eval_out_of_distribution":
            # get task types
            ...
            for json_path in file_list:
                self.json_file_list.append(data_path+"/"+json_path)
    ...
    def reset(self):
        ...
        if self.train_eval == 'train':
            ...
            tasks = [self.json_file_list[self.task_index % len(self.json_file_list)]]
    ...
    def get_frame_image(self):
        event = []
        for n in range(self.batch_size):
            event.append(self.envs[n].get_last_frame_image())
        return event
